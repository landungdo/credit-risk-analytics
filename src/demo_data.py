"""
Synthetic Lending-Club-shaped data generator.

This is the single source of truth for the demo/synthetic dataset used both by
the test fixtures (tests/conftest.py) and by the self-contained Docker build
(scripts/make_demo_data.py). Keeping it in src/ means demo/production code does
not import from the test package.

The data is deterministic (fixed seed) and constructed so the relationships the
tests assert on actually hold: higher grades default more, and recent vintages
are less resolved (right-censoring). It also emits the credit-policy-exception
status variants so the SQL/Python status-mapping test is meaningful.
"""
...
def _make_synthetic(n: int = 1200, seed: int = 7) -> pd.DataFrame:
    ...
    for i in range(n):
        ...
        if is_resolved:
            ...
            if defaulted:
                # Occasionally use the credit-policy-exception variant so the
                # SQL/Python status-mapping test actually exercises both forms.
                status = rng.choice([
                    "Charged Off",
